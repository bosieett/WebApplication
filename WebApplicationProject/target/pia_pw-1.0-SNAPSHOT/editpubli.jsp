...
<%
    ...
    PostDAO postedit=(PostDAO)session.getAttribute("postedit");
%>
...
<body class="bodycover">
    ...
    <section class="post py-6" style="padding-bottom:0;">
      <div class="container">
      <form method="POST" action="EditPubliServlet" enctype="multipart/form-data">
        <%
        ...
        <input
            name="post_edit_title"
            type="text"
            class="form-control"
            placeholder="Titulo"
            maxlength="40"
            value="<%out.print(postedit.getTitulo());%>"
            required  
          />
        
        <label class="form-control">Categoría: <% out.print(postedit.getTagname()); %></label>
        </select>
        <textarea name="post_edit_text" placeholder="¿Qué tal estuvo tu día?" spellcheck="false" maxlength="200" required><%out.print(postedit.getTexto());%></textarea>
        <input
            name="post_edit_img"
            type="file"
            class="form-control"
            value="<% out.print(postedit.getImagen()); %>"
          />
